...
def edm_sde_sampler(
    ...
    z = latents.to(dtype64) * t_steps[0]
    c = max((-d_lambda(t))**(1/2) / u(t) for t in t_steps[:-1]) + 1e-5
    for i in range(num_steps):
        sigma_t = t_steps[i]
        sigma_s = t_steps[i + 1]
        # print(f"sigma_t={sigma_t.flatten()[0].item():.5f}, sigma_s={sigma_s.flatten()[0].item():.5f}")
        h = (sigma_s - sigma_t).reshape(-1, 1, 1, 1)
        lambda_p = d_lambda(sigma_t).to(dtype64)
        denoised = net(z, sigma_t, class_labels).to(dtype64)
        eps_hat = (z - denoised) / sigma_t

        u_t = u(sigma_t).to(dtype64).reshape(-1, 1, 1, 1)
        u_t *= c
...
def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, device=torch.device('cuda'), **sampler_kwargs):
    ...
    for batch_seeds in tqdm.tqdm(rank_batches, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        sampler_fn = edm_sde_sampler if sampler_kwargs.get('sampler', 'euler') == 'euler' else vel_sde_sampler_heun
